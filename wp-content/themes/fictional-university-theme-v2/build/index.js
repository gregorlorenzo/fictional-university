...
class MyNotes {
  ...
  makeNoteEditable(thisNote) {
    ...
    this.updateNoteFieldsAccessibility(thisNote, {
      readonly: false,
      class: "note-active-field"
    });
  ...
  updateNoteFieldsAccessibility(thisNote, {
    readonly,
    class: className
  }) {
...
class Search {
  ...
  async getResults() {
    ...
    try {
      const response = await fetch(`${rootUrl}/wp-json/university/v1/search?keyword=${searchField}`);
      const results = await response.json();
      this.resultsDiv.innerHTML = `
        <div class="row">
          <div class="one-third">
